...
auto-round-light \
  ...
  ```bash
# 无需标定的优化版 RTN（iters=0，启用 opt_rtn），快速基线方案
auto-round-opt-rtn \
    --model Qwen/Qwen3-0.6B \
    --scheme "W4A16"
  ```
  ...
  ```bash
# 原始 RTN（iters=0，不做任何 AutoRound 优化），速度最快、显存占用最低
# 对受支持的 INT WOQ scheme 会自动路由到 model-free 模式
auto-round-rtn \
    --model Qwen/Qwen3-0.6B \
    --scheme "W4A16"
  ```
...
- **`diffusion_calib_gpu_resident`（bool）**：仅用于 diffusion；校准期间将整个 pipeline 的组件保留在目标设备上供不同 prompt 复用，校准结束后移回 CPU 再开始 tuning（默认 `False`）。使用 `--low_gpu_mem_usage --diffusion_calib_gpu_resident`，可减少校准时的组件搬移，同时保留低显存 tuning。显存必须容纳整个 pipeline 和校准 activation；OOM 时不会自动回退到组件 offload。每个 transformer 仍分别进行校准。
