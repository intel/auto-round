...
- ​**​`enable_torch_compile`​**（bool）：启用 `torch.compile` 以提升量化速度并降低资源消耗（默认值为 `True`）。如果编译引发兼容性问题，可在 Python 中传入 `enable_torch_compile=False`，或在命令行中使用 `--disable_torch_compile`。
