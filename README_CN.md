...
* [2026/08] 我们实验性地支持**算法组合**，例如 AWQ + SignRound（`--algorithm awq,signround`，[./docs/awq_details.md](./docs/awq_details.md)）以及 Hadamard（research 特性）+ AWQ + SignRound（`--algorithm hadamard,awq,signround`，[./docs/rotation_details.md](./docs/rotation_details.md)），以提升精度，尤其是在带有激活量化的场景中。
