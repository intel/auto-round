...
<details>
<summary>关闭low_gpu_mem_usage后的结果</summary>
...
### OPT RTN 优化舍入模式
AutoRound 同时提供优化版 RTN（Round-To-Nearest，就近舍入）模式，无需标定数据即可实现快速基线量化。**启用方式为 `iters=0`**。同时为获得更好的效果，推荐搭配 `group_size=32` 。RTN 与 OPT RTN 模式的精度对比详见[《精度对比报告》](./opt_rtn.md)。
...
通常有两种情况需要启用多 GPU 训练：一是主要为量化 lm-head 的校准阶段，二是参数量极大（如显存占用超 100GB）的模型。
...
### CUDA
支持2、3、4、8位宽量化模型，**4-bits/8-bits 推理推荐使用 GPTQModel 后端**，使用示例：
```python
from transformers import AutoModelForCausalLM, AutoTokenizer
