...
"""GPU-side fast unit tests for ``auto_round.export.utils``.

Mirrors the CPU test module to keep coverage of the shared export utility
helpers high when running the GPU test suite. These tests only need
PyTorch + a tiny dummy model and add negligible runtime to a CI partition.
"""
...
class TestGetAutogptqPackingQlinear:
    ...
    def test_returns_quant_linear_default_args(self):
        from auto_round_extension.torch.qlinear_torch_zp import QuantLinear

        result = get_autogptq_packing_qlinear("cuda")
        assert result is QuantLinear
...
class TestFilterQuantizationConfig:
    ...
    def test_quant_block_list_removed_when_empty(self):
        cfg = {"quant_block_list": [], "custom": "value"}
        result = filter_quantization_config(cfg)
        assert "quant_block_list" not in result

    def test_rotation_configs_removed(self):
        cfg = {"rotation_configs": {"key": "value"}, "custom": "value"}
        result = filter_quantization_config(cfg)
        assert "rotation_configs" not in result
