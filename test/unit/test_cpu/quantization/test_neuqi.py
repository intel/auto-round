...
class TestNeuqiFrozenInit:
    ...
    def test_zeroshot_asym_uses_joint_search(self):
        """Zero-shot optimized path, asymmetric: the wrapper resolves the
        joint (scale, zp) search quant func, and its qdq equals the direct
        search result exactly (the search IS the quant function here)."""
        from auto_round.data_type.neuqi import quant_tensor_opt_rtn_asym
        from auto_round.wrapper import WrapperLinear

        layer = _FakeQuantLayer(sym=False)
        layer.iters = 0
        wrapper = WrapperLinear(
            layer,
            device="cpu",
        ...
        assert wrapper.data_type == "opt_rtn_int_asym"
        w = layer.weight.data.to(torch.float32)
        ref_qdq, _, _ = quant_tensor_opt_rtn_asym(w, bits=layer.bits, group_size=layer.group_size)
        got_qdq, _, _ = wrapper.weight_quant_func(layer.weight.data)
        assert torch.allclose(
            got_qdq.to(torch.float32).reshape(ref_qdq.shape), ref_qdq.to(torch.float32), atol=1e-7
        ), "zero-shot asym qdq diverges from the joint search result"

        # fp16 weights: the division runs in the weight dtype (mirroring the
        # search function), so parity must hold there too
        layer_fp16 = _FakeQuantLayer(sym=False)
        layer_fp16.iters = 0
        layer_fp16.weight.data = layer_fp16.weight.data.to(torch.float16)
        wrapper_fp16 = WrapperLinear(
            layer_fp16,
            device="cpu",
            enable_minmax_tuning=False,
            enable_norm_bias_tuning=False,
        ...
        ref16, _, _ = quant_tensor_opt_rtn_asym(
            layer_fp16.weight.data, bits=layer_fp16.bits, group_size=layer_fp16.group_size
        )
        got16, _, _ = wrapper_fp16.weight_quant_func(layer_fp16.weight.data)
        assert torch.equal(got16, ref16), "fp16 zero-shot asym qdq diverges from the joint search result"

    def test_zeroshot_sym_uses_neuqi_search(self):
        """Zero-shot optimized path, symmetric: the wrapper resolves the
        two-stage NeUQI scale search (not the uniform search_scales grid),
        and its qdq equals quant_tensor_opt_rtn_sym_neuqi's result."""
        from auto_round.data_type.neuqi import quant_tensor_opt_rtn_sym_neuqi
        from auto_round.wrapper import WrapperLinear
        ...
        assert wrapper.data_type == "opt_rtn_int_sym_neuqi"
        w = layer.weight.data.to(torch.float32)
        ref_qdq, ref_scale, _ = quant_tensor_opt_rtn_sym_neuqi(w, bits=layer.bits, group_size=layer.group_size)
        got_qdq, _, _ = wrapper.weight_quant_func(layer.weight.data)
        assert torch.allclose(
            got_qdq.to(torch.float32).reshape(ref_qdq.shape), ref_qdq.to(torch.float32), atol=1e-7
        ), "zero-shot sym qdq diverges from the two-stage NeUQI search"
