...
from auto_round.utils.model import set_amax_for_all_moe_layers, get_module
        ...
        if hasattr(module, "experts") and len(list(module.experts)) > 0:
            ...
            if gate_scales and up_scales:
                ...
                for i, scale in enumerate(gate_scales):
                    assert torch.allclose(scale, gate_ref), \
                ...
                for i, scale in enumerate(up_scales):
                    assert torch.allclose(scale, up_ref), \
                ...
                if len(down_scales) > 1:
                    ...
                    if down_are_different:
                        print(f"✓ down_proj.act_max values correctly vary across experts (not unified)")
