...
def test_end_to_end_mixed_scale_dtype(tmp_path):
    ...
    * The single overridden layer → BF16 scales on disk.
    ...
    # Upstream nests single-format saves under a ``{model}-{scheme}`` pipeline
    # subfolder (ShardWriter._autoround_pipeline_subfolder), so discover shards
    # recursively rather than assuming a flat ``output_dir``.
    shards = glob.glob(f"{save_dir}/**/*.safetensors", recursive=True)
