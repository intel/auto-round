...
import torch
import transformers
from auto_round.utils import llm_load_model, mllm_load_model, get_attr, set_attr
...
def is_model_outputs_similar(model_path_1, model_path_2, metric="cosine_similarity", threshold=0.98, 
                         k=5, verbose=True):
    """
    ...
    if verbose:
        print(f"\n{'='*70}")
        print(f"Comparing Model Outputs")
        print(f"{'='*70}")
    ...
    if metric == "mse":
        ...
        passed = value >= threshold
        if verbose:
            print(f"Top-{k} Agreement: {value:.4%} | Threshold: >= {threshold:.4%} | {'✓ PASS' if passed else '✗ FAIL'}\n")
