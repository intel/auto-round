...
function run_common_unit_test() {
    ...
    for entry in $(find ./unit/common/advanced -mindepth 1 -maxdepth 1 -name 'test_evaluation_functions.py' | sort); do
        if [ -d "${entry}" ]; then
...
function run_unit_test() {
    ...
    find ./unit/test_cpu/utils -name "test*.py" | sort > all_tests.txt
