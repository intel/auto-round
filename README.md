...
* [2026/07] `torch.compile` is enabled in most scenarios to accelerate quantization, at the cost of higher RAM usage.  Minor numerical differences compared with the non-compiled path are expected due to compiler optimizations. To disable it, pass `enable_torch_compile=False` to the Python API or use `--disable_torch_compile` on the CLI.
