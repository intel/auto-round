...
- **`diffusion_calib_gpu_resident` (bool)**: Diffusion only; keep all pipeline components on the target device across calibration prompts, then move them to CPU before tuning (default is `False`). Use `--low_gpu_mem_usage --diffusion_calib_gpu_resident` to reduce component transfers during calibration while retaining low-memory tuning. Requires memory for the full pipeline and calibration activations; this option does not automatically fall back to component offload on OOM. Each transformer still has its own calibration pass.
