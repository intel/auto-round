...
class AutoRound(object):
    ...
    def quantize(self):
        ...
        #     self.model, self.layer_config, self.formats, input_ids, input_others, output, device, self.cache_device)
    ...
    def choose_one_bit(
        ...
        for layer_name in layer_names:
            ...
            set_module(block, layer_name, wrapper_layer)
            default_output = self.get_block_outputs(
                block, current_input_ids, input_others, self.batch_size * self.infer_bs_coeff, device, cache_device
            )
            set_module(block, layer_name, wrapper_layer.orig_layer)
            cur_loss = torch.sum((torch.stack((q_output - default_output) / default_config))).to(torch.float32)
            each_loss[layer_name] = cur_loss  # 把每一层的loss记录下来

        top_n_loss = sorted(each_loss.items(), key=lambda x: x[1], reverse=True)[:num_bit]  # reverse:get max
    ...
    def quant_block(self, block, input_ids, input_others, q_input=None, device=torch.device("cpu")):
        ...
        self.check_needs_auto_gguf_mix_mse(
            self.model, self.layer_config, self.formats, input_ids, input_others, output, self.device, self.cache_device
        )
        # return self.quantize_rtn()
