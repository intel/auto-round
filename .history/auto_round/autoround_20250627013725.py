...
class AutoRound(object):
    ...
    def check_needs_auto_gguf_mix_mse(self, block, layer_config, formats, input_ids, input_others, outputs, device,cache_device):
        ...
        new_config = GGUF_CONFIG["gguf:q6_k"]
        current_config = {}
        ...
        for layer_name in layer_names:
            h = block
            module = get_module(origin_block, layer_name)
            for key in new_config:  #与预设进行比较，哪个小选哪个
                setattr(module,key,default_config[key])
            wrapper_layer = WrapperLinear(module,enable_minmax_tuning=False,enable_round_tuning=False,enable_norm_bias_tuning=False,device=device)
        wrapper_block(
        origin_block, self.enable_minmax_tuning, self.enable_norm_bias_tuning, device=self.device)
        q_output = self.get_block_outputs(origin_block, current_input_ids, input_others, self.batch_size * self.infer_bs_coeff,
                                        device,
                                        cache_device)
        new_loss=mse_loss(torch.stack(q_output),current_output)
        if new_loss < default_loss:
            current_config[layer_name] = new_config
        else:
