...
class AutoRound(object):
    ...
    def check_needs_auto_gguf_mix_mse(self, block, layer_config, formats, input_ids, input_others, outputs, device,cache_device):
        ...
        self.layer_config,_ = get_layer_config_by_gguf_format(self.layer_config, f"gguf:q{min(bits)}_{split_list[2]}", self.model)
        
        if len(bits) == 2:
            self.choose_one_bit(self,block,mix_configs,quant_bits,default_config,layer_names,current_input_ids,input_others,current_output,mse_loss,device,cache_device)
        else:
            self.choose_various_bit(self,block,mix_configs,quant_bits,default_config,layer_names,current_input_ids,input_others,current_output,mse_loss,device,cache_device)
    ...
    def choose_one_bit(self,block,mix_configs,quant_bits,default_config,layer_names,current_input_ids,input_others,current_output,mse_loss,device,cache_device):
        ...
        cur_config = mix_configs.values()[0]
        num_bit = quant_bits.values()[0]
        for layer_name in layer_names:
        ...
        for _ in range(num_bit):
            # module = get_module(block, top_n_loss[1])
            # for key in cur_config:
            #     setattr(module,key,cur_config[key])
            for n in self.layer_config.keys():
                if n.endswith(top_n_loss[1]):
