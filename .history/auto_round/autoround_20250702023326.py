...
class AutoRound(object):
    ...
    def check_needs_auto_gguf_mix_mse(self, block, layer_config, formats, input_ids, input_others, outputs, device,cache_device):
        ...
        # current_input_ids = torch.stack(current_input_ids).squeeze(1)
        default_config = GGUF_CONFIG[target_gguf_format]
        # layer_configs = {target_gguf_format:GGUF_CONFIG[target_gguf_format]}
        # for mix_format in mix_formats:
        #     layer_configs[mix_format] = GGUF_CONFIG[mix_format]
        current_config = {}
        origin_block = copy.copy(block)
        # for layer_name in layer_names: #获取预设config时的输出
        ...
        #                                 device,
        #                                 cache_device)
        # default_loss=mse_loss(torch.stack(q_output),current_output)
        each_loss = {}#不同bit的总loss
        for layer_name in layer_names:
            h = block
            module = get_module(h, layer_name)
            orig = module.orig_layer
            for mix_format in mix_formats:
                cur_config = GGUF_CONFIG[mix_format]
                for key in cur_config:  #与预设进行比较，哪个小选哪个
                    setattr(orig,key,cur_config[key])
                module._init_tuning_params_and_quant_func()
                print(orig.bits)
            # wrapper_layer = WrapperLinear(module,enable_minmax_tuning=False,enable_round_tuning=False,enable_norm_bias_tuning=False,device=device)
        ...
        # block, self.enable_minmax_tuning, self.enable_norm_bias_tuning, device=self.device)
                q_output = self.get_block_outputs(h, current_input_ids, input_others, self.batch_size * self.infer_bs_coeff,
                                        device,
                                        cache_device)
                cur_loss=mse_loss(torch.stack(q_output),current_output)
                each_loss[mix_format] += cur_loss
                if new_loss < default_loss:
                    current_config[layer_name] = cur_config
                else:
                    for key in cur_config:  
                        setattr(module.orig_layer,key,default_config[key])
                    current_config[layer_name] = default_config
        breakpoint()
