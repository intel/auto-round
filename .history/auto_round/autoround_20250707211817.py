    ...
    def choose_one_bit(
        ...
        for layer_name in layer_names:
            ...
            q_output = self.get_block_outputs(
                h, current_input_ids, input_others, self.batch_size * self.infer_bs_coeff, device, cache_device
            )
        ...
        tmp_list = []
        for _ in range(quant_bits[bit_name]):  # 改成排序
            max_loss = max(zip(each_loss.values(), each_loss.keys()))
            # tmp_list.append(max_loss[1])
            h = block
            module = get_module(h, max_loss[1])
            for key in cur_config:
                setattr(module, key, cur_config[key])
            for n in self.layer_config.keys():
                if n.endswith(max_loss[1]):
                    self.layer_config[n] = cur_config
                elif self.layer_config[n]["bits"] == max(bits) and not n.endswith(max_loss[1]):
                    for q_n in layer_names:
                        if n.endswith(q_n):
                            self.layer_config[n] = low_config
                            for key in default_config:
                                setattr(module, key, default_config[key])
            each_loss.pop(max_loss[1])
