...
class AutoRound(object):
    ...
    def check_needs_auto_gguf_mix_mse(self, block, layer_config, formats, input_ids, input_others, outputs, device,cache_device):
        ...
        mix_configs = {}
        for k,_ in quant_bits:
            mix_configs[k] = GGUF_CONFIG[f"gguf:q{k}_k"]
        origin_block = copy.copy(block)
        each_loss = {}#不同bit的总loss
        layer_to_mix = {}
        for bit_name,cur_config in mix_configs:
            for layer_name in layer_names:
                h = copy.copy(origin_block)
                ...
                wrapper_block(
                h, self.enable_minmax_tuning, self.enable_norm_bias_tuning, device=self.device)
            ...
            tmp_list = []
            for i in range(quant_bits[bit_name]):
                max_loss = max(zip(each_loss.keys(),each_loss.values()))
                tmp_list.add(max_loss[2])
                del each_loss[max_loss[1]]
