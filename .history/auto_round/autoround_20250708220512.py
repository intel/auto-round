...
class AutoRound(object):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if (
            ...
            and self.low_cpu_mem_usage != True
    ...
    def _check_compatibility(self):
        ...
        if (
            ...
            and ("auto_round" in self.formats or "auto_gptq" in self.formats or "auto_awq" in self.formats)
        ):
        ...
        if not hasattr(self, "formats"):
            ...
            if only_gguf:
                self.layer_config, gguf_format_config = get_layer_config_by_gguf_format(
                    self.layer_config, self.formats, self.model
                )
    ...
    # def get_default_config(self,low_config):
    #     layer_config = {}
    #     # Get the names of layers in quantization blocks
    #     layers_in_blocks = get_layer_names_in_block(self.model, self.supported_types, self.quant_block_list)
    #     ##process regex in layer_config
    ...
    #             layer_config[n] = {}
    #             for key in keys:
    #                 layer_config[n][key] = low_config[key]
    #         # If the layer is partially configured, fill in missing values
    #         elif n in layer_config.keys():
    #             for key in keys:
    #                 if key not in layer_config[n].keys():
    ...
    def check_needs_auto_gguf_mix_mse(
        self, block, layer_config, formats, input_ids, input_others, outputs, device, cache_device
    ):
        ...
        default_layer_config = self.layer_config
        # self.set_layerwise_config(default_layer_config)
        # default_layer_config, _ = get_layer_config_by_gguf_format(self.layer_config, d_format, self.model)
    ...
    def choose_one_bit(
        ...
        for kk in top_n_loss:
            module = get_module(block, kk[0])
            for key in cur_config:
                setattr(module, key, cur_config[key])
            for n in default_layer_config.keys():
                if n.endswith(kk[0]):
                    default_layer_config[n] = cur_config
        self.layer_config = default_layer_config
