...
#
# run_sparse_sagev1_bench.sh
# ==========================
# Documented reproduce command for the sparse SAGE v1 SDPA benchmark on Intel XPU.
#
# This runs the INT8 sparse path (fp16 Q/K/V inputs, int8 quantized routing) that
# lives on the `main` branch: dense torch SDPA + dense sagev1 + the sparse
# kernel-only and sparse e2e modes, across top-k selections.
#
# WHAT IT DOES
#   Runs benchmarks/bench_sparse_topk.py (fp16 / INT8 sparse) against the rebuilt
#   XPU extension, pinning a GPU device, and saves a timestamped CSV + log under
#   benchmarks/results/.
#
# USAGE
#   ./tools/run_sparse_sagev1_bench.sh
#
# ENV OVERRIDES
...
#   BENCH_ARGS              extra args passed to bench_sparse_topk.py
#                           (e.g. "--seq-len 32768 --topk 0.5 --iters 3 --tensor-layout HND")
#
# PREREQUISITES
#   - oneAPI 2026.1 runtime (sourced from /opt/intel/oneapi/setvars.sh)
#   - Python venv at .venv/bin/python (PyTorch 2.13.0+xpu)
#   - a rebuilt extension with the sage_sparse binding under
#     auto_round_kernel/ark-xbuild/  (on `main`: rebuild after `git checkout main`)
...
#
# BENCHMARK CONFIG (bench_sparse_topk.py defaults on `main`)
#   dtype=fp16  batch=1  num_heads_q/kv=40  head_dim=128  qtile=256
#   seq_len=[32768, 75600]  tensor_layout=[HND, NHD]  topk=[0.5, 0.25, 0.125]
#   warmup=2  iters=3
#
# NOTE
#   First call per process pays a ~30s SYCL JIT for the sparse kernels; the full
#   sweep takes roughly 15-25 minutes on one GPU.
...
# Required: the triton-xpu preprocess backend is rejected by the runtime on this
# machine (SPV_INTEL_predicated_io); force the torch backend.
# export SAGE_ATTN_SPARSE_PREPROCESS_BACKEND=torch
...
out_csv="${OUTPUT_DIR}/sparse_sagev1_${stamp}.csv"
out_log="${OUTPUT_DIR}/sparse_sagev1_${stamp}.log"

echo "XPU process snapshot:"
xpu-smi ps || true
echo
echo "Running sparse SAGE v1 SDPA benchmark on device ${ZE_AFFINITY_MASK}..."
