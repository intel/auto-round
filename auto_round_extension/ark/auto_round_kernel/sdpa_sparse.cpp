...
#define ARK_SPARSE_SAGE_LAUNCHER(shape, dtype)                                                      \
  do {                                                                                              \
    if (is_causal) return detail::launch_sage_sparse_##shape##_##dtype##_causal;                     \
    if (has_mask) return detail::launch_sage_sparse_##shape##_##dtype##_noncausal_masked;            \
    return detail::launch_sage_sparse_##shape##_##dtype##_noncausal_unmasked;                        \
  } while (0)
  ...
  switch (head_dim) {
    case 128:
      if (q_tile_override == 64) {
        if (pv_dtype == BTLA_DTYPE::BF16) ARK_SPARSE_SAGE_LAUNCHER(128_qtile64, i8bf16);
        ARK_SPARSE_SAGE_LAUNCHER(128_qtile64, i8half);
      }
      if (q_tile_override != 0 && q_tile_override != 256) return nullptr;
      if (pv_dtype == BTLA_DTYPE::BF16) ARK_SPARSE_SAGE_LAUNCHER(128, i8bf16);
      ARK_SPARSE_SAGE_LAUNCHER(128, i8half);
    case 64:
      if (q_tile_override != 0 && q_tile_override != 128) return nullptr;
      if (pv_dtype == BTLA_DTYPE::BF16) ARK_SPARSE_SAGE_LAUNCHER(64, i8bf16);
      ARK_SPARSE_SAGE_LAUNCHER(64, i8half);
    default:
      return nullptr;
  }
}

#undef ARK_SPARSE_SAGE_LAUNCHER
...
void sparse_sage_prefill(sycl::queue* q, void* Q_ptr, void* K_ptr, void* V_ptr, void* O_ptr, void* mask,
  ...
  KernelLauncher launcher =
      select_sparse_sage_prefill_launcher(pv_dtype, head_dim, effective_q_tile_override, is_causal, mask != nullptr);
  if (launcher == nullptr) {
