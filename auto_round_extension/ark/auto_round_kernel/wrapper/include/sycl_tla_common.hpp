 ...
 *
 * STATUS: NEEDS-HARDWARE-VALIDATION. These `ark::` entry points are emitted by
 * the generated `sycl_tla_moe_w4a8.cpp` translation unit (MOE_SOURCE_MODE 21),
 * which forwards to the inline front end in `sycl_tla_moe_w4a8_helpers.hpp`
 * (namespace `moe_w4a8_detail`). The kernels themselves are split across the
 * per-variant TUs generated by MOE_SOURCE_MODE 22-25 so that no single
 * translation unit instantiates more than a handful of them; see
 * `sycl_tla_generation.cmake`.
 */
