...
void launch_igemm_tile(sycl::queue* q, int m, int n, int gemm_k, int lda, int ldb, const int8_t* a, const int8_t* b,
  ...
  q->parallel_for<S8DequantKernelName<AccumBlock, ElementOut, TileM, TileN, SGLayout>>(
      sycl::nd_range<2>(global, local), props, [=](auto) {
        igemm_device_impl<AccumBlock>(A, B, mma, c, accum, scale_a, scale_b, bias, m, n, block_idx, scale_b_stride);
      });
}
