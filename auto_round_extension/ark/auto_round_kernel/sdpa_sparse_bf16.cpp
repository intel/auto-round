...
KernelLauncher select_sparse_bf16_prefill_launcher(int head_dim, int q_tile_override) {
  switch (head_dim) {
    case 128:
      if (q_tile_override == 64) return launch_prefill_kernel_bf16_128_sparse_sdpa_qtile64;
...
void sparse_bf16_prefill(sycl::queue* q, void* Q_ptr, void* K_ptr, void* V_ptr, void* O_ptr, void* mask, void* lut,
                         ...
                         int head_dim, float softmax_scale, bool is_causal, int sparse_q_block_size = 0) {
  ...
  KernelLauncher launcher = select_sparse_bf16_prefill_launcher(head_dim, effective_q_tile_override);
  if (launcher == nullptr) {
    throw std::runtime_error("Unsupported sparse_bf16_prefill config");
  }
...
void sdpa_impl_bf16_sparse_d64(
    ...
    bool is_causal) {
  if (mask && is_causal) {
    throw std::invalid_argument("sdpa_impl_bf16_sparse_d64: mask and is_causal cannot both be set");
  }
  if (seq_len_q <= 0 || seq_len_kv <= 0) {
    throw std::invalid_argument("sdpa_impl_bf16_sparse_d64: seq_len_q and seq_len_kv must be greater than 0");
  }
  if (lut == nullptr || valid_block_num == nullptr) {
    throw std::invalid_argument("sdpa_impl_bf16_sparse_d64: lut and valid_block_num must be provided");
  }
  if (num_q_blocks <= 0 || num_k_blocks <= 0) {
    throw std::invalid_argument("sdpa_impl_bf16_sparse_d64: num_q_blocks and num_k_blocks must be greater than 0");
  }
  if (head_dim != 64) {
    throw std::invalid_argument("sdpa_impl_bf16_sparse_d64: head_dim must be 64");
  }

  sparse_bf16_prefill(q, Q_ptr, K_ptr, V_ptr, O_ptr, mask, lut, valid_block_num, num_q_blocks, num_k_blocks,
                      ...
                      softmax_scale, is_causal, /*sparse_q_block_size=*/64);
}

void sdpa_impl_bf16_sparse_row_linear(
    sycl::queue* q, void* Q_ptr, void* K_ptr, void* V_ptr, void* O_ptr, void* mask, void* lut, void* valid_block_num,
    int num_q_blocks, int num_k_blocks, int q_tile_override, int q_stride_s, int q_stride_d, int q_stride_h,
    int q_stride_b, int k_stride_s, int k_stride_d, int k_stride_h, int k_stride_b, int v_stride_d, int v_stride_s,
    int v_stride_h, int v_stride_b, int o_stride_s, int o_stride_d, int o_stride_h, int o_stride_b, int batch,
  ...
  if (q_tile_override != 0 && q_tile_override != 64) {
    throw std::invalid_argument("sdpa_impl_bf16_sparse_row_linear: q_tile_override must be 0 or 64");
  }
  if (mask && is_causal) {
    throw std::invalid_argument("sdpa_impl_bf16_sparse_row_linear: mask and is_causal cannot both be set");
  }
  if (seq_len_q <= 0 || seq_len_kv <= 0) {
    throw std::invalid_argument("sdpa_impl_bf16_sparse_row_linear: seq_len_q and seq_len_kv must be greater than 0");
  }
  if (lut == nullptr || valid_block_num == nullptr) {
    throw std::invalid_argument("sdpa_impl_bf16_sparse_row_linear: lut and valid_block_num must be provided");
  }
  if (num_q_blocks <= 0 || num_k_blocks <= 0) {
    throw std::invalid_argument(
        "sdpa_impl_bf16_sparse_row_linear: num_q_blocks and num_k_blocks must be greater than 0");
  }

  sparse_bf16_prefill(q, Q_ptr, K_ptr, V_ptr, O_ptr, mask, lut, valid_block_num, num_q_blocks, num_k_blocks, 64,
                      ...
                      is_causal, /*sparse_q_block_size=*/64);
}

void sdpa_impl_bf16_sparse_qtile256_row64k(
    sycl::queue* q, void* Q_ptr, void* K_ptr, void* V_ptr, void* O_ptr, void* mask, void* lut, void* valid_block_num,
    int num_q_blocks, int num_k_blocks, int q_tile_override, int q_stride_s, int q_stride_d, int q_stride_h,
    int q_stride_b, int k_stride_s, int k_stride_d, int k_stride_h, int k_stride_b, int v_stride_d, int v_stride_s,
    int v_stride_h, int v_stride_b, int o_stride_s, int o_stride_d, int o_stride_h, int o_stride_b, int batch,
  ...
  if (q_tile_override != 256) {
    throw std::invalid_argument("sdpa_impl_bf16_sparse_qtile256_row64k: q_tile_override must be 256");
  }
  if (head_dim != 128) {
    throw std::invalid_argument("sdpa_impl_bf16_sparse_qtile256_row64k: head_dim must be 128");
  }
  ...
  sparse_bf16_prefill(q, Q_ptr, K_ptr, V_ptr, O_ptr, mask, lut, valid_block_num, num_q_blocks, num_k_blocks,
                      ...
                      k_stride_h, k_stride_b, v_stride_d, v_stride_s, v_stride_h, v_stride_b, o_stride_s, o_stride_d,
                      o_stride_h, o_stride_b, batch, num_heads_q, num_heads_kv, seq_len_q, seq_len_kv, head_dim,
                      softmax_scale, is_causal, /*sparse_q_block_size=*/256);
}
