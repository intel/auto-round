...
struct f8e4m3 {
  int8_t x;
  explicit f8e4m3(int8_t v) { x = v; }
  f8e4m3& operator=(int8_t v) {
    x = v;
    return *this;
  }

  inline float tofloat() const {
    uint8_t value = bit_cast<uint8_t, int8_t>(x);
    uint32_t sign_revert = value;
    uint32_t e_revert = value;
    uint32_t mantissa_revert = value;
    sign_revert <<= 24;
    sign_revert &= 0x80000000;
    int constexpr ebits = 4;
    auto mantissabit = 7 - ebits;
    e_revert &= 0x7f;
    e_revert >>= mantissabit;
    e_revert = e_revert + 1 + 127 - static_cast<uint32_t>(std::pow(2, ebits - 1));
    e_revert <<= 23;
    mantissa_revert <<= (23 - mantissabit);
    mantissa_revert &= 0x007fffff;
    uint32_t tmp = sign_revert | e_revert | mantissa_revert;
    return bit_cast<float, uint32_t>(tmp);
  }
...
static inline size_t remainsize(size_t pos, size_t size, size_t N) { return pos + N <= size ? N : size - pos; }
static inline size_t remainsize(size_t pos, int size, size_t N) { return pos + N <= size ? N : size - pos; }
...
template <typename T>
static inline T padto(T a, int b) {
  return (a + b - 1) / b * b;
}
