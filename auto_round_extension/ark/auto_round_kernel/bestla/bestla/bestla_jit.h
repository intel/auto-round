...
class JitAvx : protected JitBase {
 protected:
  static auto constexpr ISA = BTLA_ISA::AVX;
...
class JitAmxfp16 : protected JitAmxtile {
 public:
  static auto constexpr ISA = BTLA_ISA::AMX_FP16;

 protected:
  void cvt_fp32_fp16(const Xbyak::Ymm& _fp16, const Xbyak::Zmm& _fp32) { vcvtps2phx(_fp16, _fp32); }
};
...
class JitInfo {
 public:
  ...
  static bool constexpr Has512 = ISA >= BTLA_ISA::AVX512F;
  static bool constexpr HasTile = ISA >= BTLA_ISA::AMX_BF16;
  static int constexpr VBits = Has512 ? 512 : 256;
  static int constexpr VecBytes = VBits / 8;
  static int constexpr VecLen32 = VBits / 32;
  static int constexpr RegCount = Has512 ? 32 : 16;
  static int constexpr TileCount = HasTile ? 8 : 0;
  using vreg_t = std::conditional_t<Has512, Xbyak::Zmm, Xbyak::Ymm>;
  using hvreg_t = std::conditional_t<Has512, Xbyak::Ymm, Xbyak::Xmm>;
  using qvreg_t = Xbyak::Xmm;
};

class JitCode : protected Xbyak::CodeGenerator {
 public:
  BTLA_ISA ISA;

 protected:
  JitCode(size_t size = 16 * 1024) : CodeGenerator(size) {}
  ...
  void vpdpbusds_(const Xbyak::Xmm& x1, const Xbyak::Xmm& x2, const Xbyak::Operand& op,
                  const Xbyak::Xmm& sum2 = Xbyak::Xmm{}, const Xbyak::Xmm& ones = Xbyak::Xmm{}) {
    if (ISA == BTLA_ISA::AVX512_VNNI) {
      vpdpbusds(x1, x2, op, Xbyak::EvexEncoding);
    } else if (ISA == BTLA_ISA::AVX_VNNI) {
      vpdpbusds(x1, x2, op, Xbyak::VexEncoding);
    } else {
      vpmaddubsw(sum2, x2, op);
      vpmaddwd(sum2, sum2, ones);
      vpaddd(x1, x1, sum2);
    }
  }

  void vxor(const Xbyak::Xmm& x1, const Xbyak::Xmm& x2, const Xbyak::Operand& op) { vxorps(x1, x2, op); }

  template <typename TA, typename TB>
  void _tdpb(const Xbyak::Tmm& x1, const Xbyak::Tmm& x2, const Xbyak::Tmm& x3) {
    assert(ISA >= BTLA_ISA::AMX_BF16);
    if (std::is_same_v<TA, uint8_t>) {
      if (std::is_same_v<TB, int8_t>) {
        tdpbusd(x1, x2, x3);
      } else {
        tdpbuud(x1, x2, x3);
      }
