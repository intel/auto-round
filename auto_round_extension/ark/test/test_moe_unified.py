...
This file checks:
  ...
  * Dispatch correctness: ``phase="auto"`` picks decode when total tokens are
    below threshold and prefill otherwise.
...
# ---------------------------------------------------------------------------
# Small shapes (one decode-sized by total tokens, one prefill-sized by total
...
class TestMoeUnifiedDispatch:
    ...
    def test_auto_picks_decode_for_small_total_tokens(self):
        shape = _AUTO_DECODE_SHAPE
    ...
    def test_decode_threshold_override(self):
        decode_shape = _AUTO_DECODE_SHAPE
        decode_total_tokens = sum(decode_shape["tokens_per_expert"])
        E, N, K = decode_shape["num_experts"], decode_shape["N"], decode_shape["K"]
        group_size = 128
        dtype = torch.float16
        ...
        prefill_shape = _AUTO_PREFILL_SHAPE
        prefill_total_tokens = sum(prefill_shape["tokens_per_expert"])
        activations, packed, scales, _ = _make_int4_sym(E, N, K, group_size, dtype, prefill_total_tokens)
        ntpe = torch.tensor(prefill_shape["tokens_per_expert"], dtype=torch.int32, device="xpu")
        ...
        out_auto = ark.moe(
            ...
            asym=False,
            phase="auto",
            decode_threshold=prefill_total_tokens,
        )
        out_decode = ark.moe_gemm_decode(
            activations,
            packed,
    ...
    def test_decode_threshold_env_override(self, monkeypatch):
        shape = _AUTO_PREFILL_SHAPE
        total_tokens = sum(shape["tokens_per_expert"])
        E, N, K = shape["num_experts"], shape["N"], shape["K"]
        group_size = 128
        ...
        monkeypatch.setenv("ARK_MOE_AUTO_DECODE_MAX_TOKENS", "512")
        out_auto = ark.moe(
            activations,
            packed,
            ntpe,
        ...
        monkeypatch.setenv("ARK_MOE_AUTO_DECODE_MAX_TOKENS", "invalid")
        out_auto = ark.moe(
            activations,
            packed,
            ntpe,
...
class TestMoeUnifiedBitParity:
    ...
    @pytest.mark.parametrize(
        "shape_name,shape",
        [
            ("decode-shape", _AUTO_DECODE_SHAPE),
            ("prefill-shape", _AUTO_PREFILL_SHAPE),
        ],
    )
