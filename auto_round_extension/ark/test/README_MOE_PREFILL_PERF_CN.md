...
| 优先级       | Env 开关                                                                        | Kernel                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ------------ | ------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1 (最高)     | `ARK_MOE_PREFILL_DPAS_S4=1`(**默认关闭**)                        | **S4-sym 单遍 DPAS 混合输入 mainloop**。直接读取 packed `[E, N, K/2]` `uint8_t` nibble,在寄存器中把 S4 上转到 `act_dtype`。每个 `int4b_t` B fragment 先解码为 `int8_t` 暂存 fragment,再交给与 INT8 路径共享的、已验证的 `int8_t → act_dtype` `reorder(...)`(从而绕开会假设 B 已预置换布局的交织式 `NumericArrayConverter<ElementA, cutlass::int4b_t, N>` 快路径,而 auto-round 并不产生这种置换)。B 侧 global 带宽正好是 S8 路径的一半。Per-K-group scale 使用与 INT8 相同的组边界延迟折叠。实现于 `sycl_tla_moe_prefill_s4_dpas.hpp`。**状态:NEEDS-HARDWARE-VALIDATION —— 因在生产规模 prefill shape 上仍偶发少量输出算错而默认关闭,仅供调试时手动开启。** |
