...
def test_public_mixed_sdpa_packed_path_forwards_extended_features(monkeypatch):
    q = torch.randn(1, 2, 1, 16, dtype=torch.float32)
    k = torch.randn(1, 1, 8, 16, dtype=torch.float16)
    v = torch.randn(1, 1, 8, 16, dtype=torch.float16)
    captured = {}
    expected = torch.empty_like(q)

    def packed_call(*args, **kwargs):
        captured.update(kwargs)
        return expected

    monkeypatch.setattr(auto_round_kernel, "_cpu_public_packed_kv_available", lambda: True)
    monkeypatch.setattr(auto_round_kernel, "_cpu_public_mixed_sdpa_packed", packed_call)
