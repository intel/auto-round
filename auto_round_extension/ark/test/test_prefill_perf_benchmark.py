...
try:
    ...
    from test_moe import (
        _pack_fp8, _pack_int2_asym, _pack_int2_sym, 
        _pack_int4_asym, _pack_int4_sym, _pack_int8_asym, _pack_int8_sym,
    )
...
WEIGHT_FORMATS = [
    "int8_sym", "int8_asym", "int4_sym", "int4_asym", 
    "int2_sym", "int2_asym", "fp8_e4m3", "fp8_e5m2"
]
...
def _make_full_pad_clip_activations(activations, tokens_per_expert, offsets, threshold, pad_len):
    ...
    padded_act = torch.zeros((NUM_EXPERTS * pad_len, activations.shape[1]), 
                            dtype=activations.dtype, device="xpu")
    ...
    for expert_id, count in enumerate(tokens_per_expert):
        ...
        padded_act[dst_start:dst_start + take].copy_(
            activations[src_start:src_start + take]
        )
...
def run_full_pad_clip_quant(activations, weights, scales, zeros, weight_bits, asym, 
                           tokens_per_expert, threshold):
    ...
    def _run():
        # Big GEMM: all experts padded/clipped
        _call_quant_moe_gemm(padded_activations, weights, num_tokens_per_expert, 
                            scales, zeros, weight_bits, asym)
        ...
        for expert_id, count in enumerate(tokens_per_expert):
            if count > effective_threshold:
                ...
                remaining_nt = torch.tensor([remaining], dtype=torch.int32, device="xpu")
                _call_quant_moe_gemm(remaining_acts, remaining_w, remaining_nt, 
                                   remaining_s, remaining_z, weight_bits, asym)
...
def main():
    ...
    for weight_format in WEIGHT_FORMATS:
        try:
            ...
            # Baseline timing
            baseline_ms = run_baseline_quant(activations, packed, scales, zeros, 
                                           weight_bits, asym, tokens_per_expert)
            
            # Test each threshold
            for threshold in THRESHOLDS:
                padclip_ms = run_full_pad_clip_quant(activations, packed, scales, zeros, 
                                                   weight_bits, asym, tokens_per_expert, threshold)
                speedup = baseline_ms / padclip_ms if padclip_ms > 0 else float('inf')
