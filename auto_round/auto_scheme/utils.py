...
def _log_score_summary_by_block_and_nonblock(
    ...
    for name, avg_loss in rows_in_order:
        ...
        logger.debug("AutoScheme | %d | %s | %.4e | %.2f%% |", rank, name, avg_loss, ratio_pct)
    ...
    if non_block_items:
        ...
        for layer_name, layer_loss in non_block_items:
            logger.info("AutoScheme non_block=%s loss=%.4e", layer_name, layer_loss)
...
def _log_batch_avg_loss(model, batch_idx: int, pbar=None, block_names=None, total_batches=None, scheme_tag=None):
    ...
    msg = f"AutoScheme {tag}cumulative batch {batch_str}  avg_loss={avg_loss:.4e} layers={layer_cnt}"
    if pbar is not None:
        pbar.write(msg)
    logger.debug(msg)
