    ...
    **clears** block weights from memory and **reloads** them on demand from the
    ...
    def _register_hooks(self, model: torch.nn.Module, block_names: list[str]) -> None:
        ...
        for block_name in block_names:
            ...
            if block is None:
                continue
            handle = block.register_forward_pre_hook(partial(self._pre_forward_hook, block_name=block_name))
...
class BlockOffloadManager:
    ...
    # Stream offload
    # ------------------------------------------------------------------
    def stream_offload_all_blocks(self, model: torch.nn.Module, all_blocks: list[list[str]], device_list=None) -> None:
