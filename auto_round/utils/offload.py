...
class AutoSchemeOffloadContext:
    ...
    **clears** block weights from memory and **reloads** them on demand from the
    ...
    writing any extra data to disk — the original model files already contain
    everything needed.

    For wrapped (quantization-scheme-applied) blocks the state can be cheaply
    regenerated by re-wrapping from original weights, so those are simply
    ...
    def __init__(self, low_cpu_mem_usage: bool = False, model_dir: str | None = None):
        ...
        self._cleared_blocks: set[str] = set()  # track which blocks have been cleared
    ...
    # --- Original (unwrapped) weight management ---
    def clear_all_original_blocks(self, model: torch.nn.Module, block_names: list[str]) -> None:
        """Clear all block weights from memory (they can be reloaded from model files)."""
        if not self.low_cpu_mem_usage:
            return
        logger.info("AutoScheme: clearing original block weights to free RAM...")
    ...
    def load_original_block_weights(self, block_name: str, block: torch.nn.Module) -> None:
        """Reload a block's original weights from the model checkpoint files."""
        if not self.low_cpu_mem_usage or self._model_dir is None:
            return
    ...
    # --- Wrapped-state management ---
    def offload_block_weights(self, block_name: str, block: torch.nn.Module) -> None:
        """Clear wrapped block weights (they will be re-generated from originals)."""
        if not self.low_cpu_mem_usage:
            return
        self._clear_block(block)
        self._cleared_blocks.add(block_name)
    ...
    def load_block_weights(self, block_name: str, block: torch.nn.Module) -> None:
        """Reload a block's original weights from model checkpoint files.

        For the forward/backward pass, wrapped blocks need their original weights
        back so that WrapperLinear can re-compute.  Since wrapped layers hold a
        reference to their ``orig_layer``, we just need to reload into
        ``orig_layer`` — the wrapper reads through it automatically.
        """
        if not self.low_cpu_mem_usage or self._model_dir is None:
            return
        load_block_from_model_files(self._model_dir, block_name, block)
    ...
    def cleanup(self) -> None:
        """No temp files to remove — just reset tracking state."""
        self._cleared_blocks.clear()
