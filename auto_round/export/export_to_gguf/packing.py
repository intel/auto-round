...
def q2_k_quant_block(blocks, scale=None, wmin=None, d_scale=None, d_wmin=None, imatrix=None, original=False, **kwargs):
    ...
    if scale is not None:
        ...
        output_scale = torch.round(scales / output_d).clip(0, 15).to(torch.uint8)
        output_scale |= torch.round(mins / output_dmin).clip(0, 15).to(torch.uint8) << 4
        ...
        id_scales = torch.where(max_scales > 0, 15. / max_scales, 0)
        id_mins = torch.where(max_mins > 0, 15. / max_mins, 0)

        replace_ids = (max_scales > 0).squeeze()
        output_scale = torch.zeros_like(scales).to(torch.uint8)
        output_scale[replace_ids] = torch.round(
            id_scales[replace_ids] * scales[replace_ids]).clip(0, 15).to(torch.uint8)

        replace_ids = (max_mins > 0).squeeze()
        output_scale[replace_ids] |= torch.round(
            id_mins[replace_ids] * mins[replace_ids]).clip(0, 15).to(torch.uint8) << 4
        ...
        all_L[replace_ids] = torch.round(
            (blocks[replace_ids] + dm_tmp[replace_ids].unsqueeze(-1)) / d_tmp[replace_ids].unsqueeze(-1)).clip(0, 3).to(torch.uint8)
    else:
...
@register_qtype("q3_k")
def q3_k_quant_block(blocks: np.array, scale=None, d_scale=None, original=None, imatrix=None, **kwargs):
    nb = blocks.shape[0]
    blocks = blocks.reshape(nb, QK_K // 16, 16)
    ...
    if scale is not None:
        ...
        all_L = (torch.round(blocks / d_scale.unsqueeze(-1)).clip(-4, 3) + 4).to(torch.uint8)
        q_scales_offset = torch.round(qdq_scale / dq_scale).clip(-32, 31) + 32
        ...
        blocks.reshape(blocks.shape[0], -1)
        blocks, scales, mins = quant_tensor_gguf_sym_dq(blocks, bits=3, scale_dtype=torch.float32, imatrix=imatrix)
        blocks = blocks.reshape((nb, QK_K // 32, 32))
        qdq_scale = scales.reshape((-1, QK_K // 32))
        all_L = (torch.round(blocks / d_scale.unsqueeze(-1)).clip(-4, 3) + 4).to(torch.uint8)
        q_scales_offset = torch.round(qdq_scale / dq_scale).clip(-32, 31) + 32
...
def q4_k_quant_block(blocks, scale=None, wmin=None, d_scale=None, d_wmin=None, imatrix=None, original=False, **kwargs):
    ...
    if scale is not None:
        ...
        output_d = d_scale.reshape(-1, 1).to(torch.float32)
        output_dmin = d_wmin.reshape(-1, 1).to(torch.float32)
        q_scales = torch.round(scales / output_d).clip(0, 63).to(torch.uint8)
        q_mins = torch.round(mins / output_dmin).clip(0, 63).to(torch.uint8)
        all_L = torch.round((blocks + mins.unsqueeze(-1)) / scales.unsqueeze(-1)).clip(0, 15).to(torch.uint8)
    elif original:
        scales, all_L, mins = make_qkx2_quants(blocks, bits=4, rmin=-1, rdelta=0.1, nstep=20, use_mad=False)
        max_scales = torch.max(scales, dim=-1, keepdim=True)[0]
        max_mins = torch.max(mins, dim=-1, keepdim=True)[0]
        id_scales = torch.where(max_scales > 0, 63. / max_scales, 0)
        id_mins = torch.where(max_mins > 0, 63. / max_mins, 0)
        ...
        replace_ids = d_tmp != 0
        all_L[replace_ids] = torch.round((blocks[replace_ids] + dm_tmp[replace_ids].unsqueeze(-1)) /
                                         d_tmp[replace_ids].unsqueeze(-1)).clip(0, 15).to(torch.uint8)
    else:
        from auto_round.data_type.gguf import quant_tensor_gguf_asym_dq
        blocks.reshape(blocks.shape[0], -1)
...
@register_qtype("q5_k")
def q5_k_quant_block(blocks, scale=None, zp=None, wmin=None, d_scale=None, d_wmin=None, imatrix=None, original=False, **kwargs):
    nb = blocks.shape[0]
    blocks = blocks.reshape((nb, QK_K // 32, 32))
    ...
    if scale is not None:
        ...
        q_scales = torch.round(scales / output_d).clip(0, 63).to(torch.uint8)
        q_mins = torch.round(mins / output_dmin).clip(0, 63).to(torch.uint8)
        all_L = torch.round((blocks + mins.unsqueeze(-1)) / scales.unsqueeze(-1)).clip(0, 31).to(torch.uint8)
    elif original:
    ...
    output_qh = np.bitwise_or.reduce(
        output_qh.cpu().numpy(), axis=1, dtype=np.uint8).astype(np.uint8)  # pylint: disable=E1121
...
@register_qtype("q6_k")
def q6_k_quant_block(blocks: np.array, scale=None, d_scale=None, original=None, imatrix=None, **kwargs):
    nb = blocks.shape[0]
    blocks = blocks.reshape((nb, QK_K // 16, 16))
    ...
    if scale is not None:
        ...
        output_scale = torch.round(scales / output_d).clip(max=127).to(torch.int8)
        all_L = torch.round(blocks / scales.unsqueeze(-1) + 32).clip(0, 63).to(torch.uint8)
    elif original:
    ...
    ouptut_qh = (all_L >> 4).reshape(nb, 2, 4, 32) << torch.tensor(
        [0, 2, 4, 6], device=all_L.device).reshape(1, 1, 4, 1)
    output_qh = np.bitwise_or.reduce(
        ouptut_qh.cpu().numpy(),  # pylint: disable=E1121
        axis=2, dtype=np.uint8).reshape(nb, QK_K // 4).astype(np.uint8)  # pylint: disable=E1121
