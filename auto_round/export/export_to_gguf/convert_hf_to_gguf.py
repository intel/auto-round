...
try:
    ...
    from mistral_common.tokens.tokenizers.multimodal import (
        DATASET_MEAN as _MISTRAL_COMMON_DATASET_MEAN,  # pyright: ignore[reportMissingImports]
    )
    from mistral_common.tokens.tokenizers.multimodal import DATASET_STD as _MISTRAL_COMMON_DATASET_STD
...
class ModelBase:
                ...
                if bits in [2, 4, 8]:
                    ...
                    weight = torch.bitwise_and(
                        torch.bitwise_right_shift(
                            qweight.unsqueeze(1).expand(-1, pack_factor, -1), wf.unsqueeze(-1)
                        ).to(torch.int16 if bits == 8 else torch.int8),
                        maxq,
                    )
                elif bits == 3:
                ...
                for name in self.model_tensors.keys():
                    if name.endswith(".qweight"):
                        ...
                        new_tensors[base_name + ".weight"] = (
                            lambda g=g_idx, z=qzeros, w=qweight, s=scales: dequant_gptq(g(), w(), z(), s())
                        )
                        tensors_to_remove += [
                            base_name + n
                            for n in (
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        if self.fuse_gate_up_exps and bid is not None:
            ...
            ) or self.match_model_tensor_name(new_name, gguf.MODEL_TENSOR.FFN_UP_EXP, bid):
                return []
    ...
    def _generate_nvfp4_tensors(self):
        ...
        for name in self.model_tensors.keys():
            ...
            input_scale = LazyTorchTensor.to_eager(
                self.model_tensors.get(input_scale_name, lambda: torch.tensor(1.0))()
            )
            ...
            if m:
                ...
                if n_experts > 0 and len(expert_blocks[key]) >= n_experts:
                    self._flush_nvfp4_experts(
                        key, expert_blocks, expert_scales, expert_input_scales, expert_shapes, bid, proj_type
                    )
            else:
        ...
        for bid, proj_type in list(expert_blocks.keys()):
            self._flush_nvfp4_experts(
                (bid, proj_type), expert_blocks, expert_scales, expert_input_scales, expert_shapes, bid, proj_type
            )
    ...
    def _flush_nvfp4_experts(
        self, key, expert_blocks, expert_scales, expert_input_scales, expert_shapes, bid, proj_type
    ):
    ...
    def prepare_tensors(self):
        # Detect NVFP4 / MXFP4 from quantization config
        hf_quant_config_path = self.dir_model / "hf_quant_config.json"
        ...
        if hf_quant_config_path.exists():
            ...
            quant_cfg = hf_quant_config.get("quantization", {})
            quant_type = quant_cfg.get("quant_type", "")
            if quant_type == "nvfp4":
                self._is_nvfp4 = True
            elif quant_type == "mxfp4":
                self._is_mxfp4 = True
        elif quantization_config.get("quant_method") == "modelopt":
            # ModelOpt NVFP4 models don't always have hf_quant_config.json
            # Check by presence of *.weight_scale_2 tensors in any shard
            for name in self.model_tensors.keys():
...
class TextModel(ModelBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        local_rope_theta = self.find_hparam(
            ["local_rope_theta", "rope_local_theta", "rope_theta_local", "swa_rope_theta", "rope_local_base_freq"],
            optional=True,
        )
    ...
    def set_gguf_parameters(self):
        ...
        if (rope_type := rope_params.get("rope_type")) is not None:
            ...
            if rope_type == "linear" and rope_factor is not None:
                ...
                if (
                    yarn_attn_factor := rope_params.get("attention_factor", rope_params.get("attn_factor"))
                ) is not None:
        ...
        if (
            n_experts_used := self.find_hparam(["num_experts_per_tok", "num_experts_per_token"], optional=True)
        ) is not None:
        ...
        if (
            score_func := self.find_hparam(
                ["score_function", "scoring_func", "score_func", "moe_router_activation", "moe_router_activation_func"],
                optional=True,
            )
        ) is not None:
                ...
                if token in added_vocab:
                    ...
                    if not added_tokens_decoder[i].normalized:
                        ...
                        if previous_token != token:
                            logger.info(
                                f"{repr(previous_token)} is encoded and decoded back to {repr(token)} using"
                                " AutoTokenizer"
                            )
    ...
    def get_vocab_base_pre(self, tokenizer) -> str:
        ...
        chktxt = (
            ...
            " 🦙🦙 3 33 333 3333 33333 333333 3333333 33333333 3.3 3..3 3...3 កាន់តែពិសេសអាច😁"
        ...
        if chkhsh == "d30d75d9059f1aa2c19359de71047b3ae408c70875e8a3ccf8c5fba56c9d8af4":
            ...
            res = "qwen35"
        if chkhsh == "29f3cefa2c16eacb80686a3286fd9ab13ccc0f7b87a88427143e31db3fb08ca3":
    ...
    def _set_vocab_mistral(self):
        ...
        logger.info(f"Converting tokenizer {vocab.tokenizer_type} of size {vocab.vocab_size}.")
        ...
        assert len(tokens) == vocab.vocab_size, f"token count ({len(tokens)}) != vocab size ({vocab.vocab_size})"
        ...
        if self.is_mistral_format and local_template_file_path.is_file():
            ...
            if self.is_mistral_format:
                logger.info(
                    "Using a Mistral community chat template. These templates can be subject to errors in early days or"
                    " weeks after a release. Mistral recommends to use `mistral-common` to perform tokenization and"
                    " detokenization."
                )
            template = MistralModel.get_community_chat_template(vocab, template_dir, self.is_mistral_format)
        else:
            logger.info(
                "Not using a Mistral local or community chat template. Ensure to perform the tokenization and"
...
class MmprojModel(ModelBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self.is_mistral_format:
            ...
            text_config = {k: v for k, v in self.hparams.items() if k not in ["vision_encoder", "audio_encoder"]}
    ...
    def set_gguf_parameters(self):
        ...
        if self.has_vision_encoder:
            ...
            self.gguf_writer.add_vision_head_count(
                self.find_vparam(["num_attention_heads", "num_heads", "vt_num_attention_heads"])
            )

            # preprocessor config
            image_mean = (
                _MISTRAL_COMMON_DATASET_MEAN if self.is_mistral_format else self.preprocessor_config["image_mean"]
            )
                ...
                if len(self._experts[bid]) >= n_experts * 3:
                    ...
                    for wid in [("linear", "w1", 0), ("linear_1", "w2", 1), ("linear_v", "w3", 0)]:
                        ...
                        for xid in range(n_experts):
                            ...
                            data.append(torch.cat(tensor_list, dim=wid[2]) if len(tensor_list) > 1 else tensor_list[0])
                            del self._experts[bid][ename]
...
class Phi3MiniModel(TextModel):
    ...
    def generate_extra_tensors(self) -> Iterable[tuple[str, Tensor]]:
        ...
        if len(long_factors) != len(short_factors) or len(long_factors) != rope_dims / 2:
            raise ValueError(
                f"The length of rope long and short factors must be {rope_dims / 2}. long_factors ="
                f" {len(long_factors)}, short_factors = {len(short_factors)}."
            )
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        if name.endswith((".q_conv1d.weight", ".k_conv1d.weight", ".v_conv1d.weight")):
            ...
            if data_torch.ndim == 2:
                ...
                logger.info(
                    f"Reshaped conv1d weight {name}: [d_inner={d_inner}, d_conv={d_conv}] -> numpy"
                    f" {tuple(data_torch.shape)} -> ggml ne=[{d_conv}, 1, {d_inner}, 1]"
                )
                ...
                data_torch = data_torch.reshape(1, d_inner, 1, d_conv)
                logger.info(
                    f"Reshaped conv1d weight {name}: [d_inner={d_inner}, 1, d_conv={d_conv}] -> numpy"
                    f" {tuple(data_torch.shape)} -> ggml ne=[{d_conv}, 1, {d_inner}, 1]"
                )
                        ...
                        if model_tensors_file.is_file():
                            ...
                            if mod_conf_file.is_file():
                                with open(mod_conf_file, encoding="utf-8") as mod_conf_json_file:
                                    ...
                                    # hparams dense_2_feat_out and dense_3_feat_in are required
                                    # when loading model's dense weights
...
class Gemma3nVisionAudioModel(ConformerAudioModel):
    ...
    block_tensor_mapping = {
        ...
        "model.vision_tower.timm_model.blocks.{bid}.{sid}.dw_start.conv.weight": (
            "v.blk.{bid}.{sid}.dw_start.conv.weight"
        ),
        ...
        "model.vision_tower.timm_model.blocks.{bid}.{sid}.attn.query.proj.weight": (
            "v.blk.{bid}.{sid}.attn.query.proj.weight"
        ),
        "model.vision_tower.timm_model.blocks.{bid}.{sid}.attn.key.proj.weight": (
            "v.blk.{bid}.{sid}.attn.key.proj.weight"
        ),
        "model.vision_tower.timm_model.blocks.{bid}.{sid}.attn.value.proj.weight": (
            "v.blk.{bid}.{sid}.attn.value.proj.weight"
        ),
        "model.vision_tower.timm_model.blocks.{bid}.{sid}.attn.output.proj.weight": (
            "v.blk.{bid}.{sid}.attn.output.proj.weight"
        ),
        "model.vision_tower.timm_model.blocks.{bid}.{sid}.attn.key.down_conv.weight": (
            "v.blk.{bid}.{sid}.attn.key.down_conv.weight"
        ),
        "model.vision_tower.timm_model.blocks.{bid}.{sid}.attn.key.norm.weight": (
            "v.blk.{bid}.{sid}.attn.key.norm.weight"
        ),
        "model.vision_tower.timm_model.blocks.{bid}.{sid}.attn.value.down_conv.weight": (
            "v.blk.{bid}.{sid}.attn.value.down_conv.weight"
        ),
        "model.vision_tower.timm_model.blocks.{bid}.{sid}.attn.value.norm.weight": (
            "v.blk.{bid}.{sid}.attn.value.norm.weight"
        ),
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        if name.startswith("model.vision_tower.timm_model.blocks."):
            ...
            # Route non-repeating (conv_stem, msfa, embedding, etc.) and un-caught through tensor_mapping.py
            new_name = self.map_tensor_name(name)
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        if "embed_tokens.weight" in name or "embed_tokens_per_layer" in name:
            ...
            if current_size < vocab_size:
                ...
                logger.info(
                    f"Padding {tensor_type} shape {list(data_torch.shape)} from {current_size} to {vocab_size} (adding"
                    f" {padding_size} vision/audio token slots)"
                )
            ...
            with open(tokenizer_config_file, "r", encoding="utf-8") as f:
                ...
                if "added_tokens_decoder" in tokenizer_config_json:
                    ...
                    for token_id, token_json in added_tokens_decoder.items():
                        ...
                        logger.info(
                            f"Setting added token {token_id} to '{token_content}' (type: {token_type}, score:"
                            f" {token_score:.2f})"
                        )
...
class DeepseekV2Model(TextModel):
    ...
    def set_gguf_parameters(self):
        ...
        if (rope_mscale_all := self.rope_parameters.get("mscale_all_dim")) is not None:
            ...
            # note: for legacy reasons, this is not consistent with the other usages
            # of self.gguf_writer.add_rope_scaling_yarn_log_mul
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None):
        ...
        if "experts" in name:
            ...
            for w_name in expert_weights:
                ...
                for xid in range(n_experts):
                    ename = f"model.layers.{bid}.block_sparse_moe.experts.{xid}.{w_name}.weight"
                    data.append(expert_cache[ename])
...
class MimoV2Model(TextModel):
    ...
    def set_gguf_parameters(self):
        ...
        n_head_kv_arr = [
            n_head_kv_swa if use_swa == 1 else n_head_kv for use_swa in self.hparams["hybrid_layer_pattern"]
        ]
    ...
    def modify_tensors(self, data_torch, name, bid):
        ...
        # TODO: mimo v2 does not indicate the number of next-token-prediction layers,
        # therefore we cannot do the same way as GLM4_MOE
...
class Step35Model(TextModel):
    ...
    def generate_extra_tensors(self) -> Iterable[tuple[str, Tensor]]:
        ...
        old_context_len = int(
            rope_params.get(
                "original_max_position_embeddings", self.hparams.get("original_max_position_embeddings", 8192)
            )
        )
...
class T5Model(TextModel):
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        # "encoder.embed_tokens.weight", "decoder.embed_tokens.weight" or "shared.weight" tensor.
        # In some models there are even multiple of them stored in the safetensors files.
        # We use the first tensor from these three as the token embeddings for both encoder
...
class NemotronHModel(GraniteHybridModel):
    ...
    def get_attn_layers(self):
        hybrid_override_pattern = self.hparams["hybrid_override_pattern"]
        assert (
            len(hybrid_override_pattern) == self.block_count
...
class BailingMoeModel(TextModel):
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        if name.endswith("attention.dense.weight"):
            ...
            yield from super().modify_tensors(
                BailingMoeModel.permute(q, n_head, n_head), self.format_tensor_name(gguf.MODEL_TENSOR.ATTN_Q, bid), bid
            )
            yield from super().modify_tensors(
...
class BailingMoeV2Model(TextModel):
    ...
    def set_gguf_parameters(self):
        ...
        self.gguf_writer.add_expert_shared_feed_forward_length(
            hparams.get(
                "moe_shared_expert_intermediate_size", hparams["moe_intermediate_size"] * hparams["num_shared_experts"]
            )
        )
            ...
            if len(self._chunk_experts[bid]) >= n_experts * 3:
                ...
                for w_name in ["down_proj", "gate_proj", "up_proj"]:
                    ...
                    for xid in range(n_experts):
                        ...
                        data.append(self._chunk_experts[bid][ename])
...
class UltravoxModel(TextModel):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        raise NotImplementedError(
            "Ultravox does not have text decoder. Instead, it uses Llama or other models for text. If you want to get"
            " the audio encoder, please use --mmproj argument"
        )
...
class MistralModel(LlamaModel):
    ...
    def get_community_chat_template(vocab: MistralVocab, templates_dir: Path, is_mistral_format: bool):
        ...
        if vocab.tokenizer.version == TokenizerVersion.v1:
            ...
            if is_mistral_format:
                err_message += (
                    " . Please pass --disable-mistral-community-chat-template argument to the CLI if you want to skip"
                    " this error and use the Mistral official `mistral-common` pre-processing library."
                )
...
class PixtralModel(LlavaVisionModel):
    ...
    def set_gguf_parameters(self):
        ...
        self.gguf_writer.add_vision_attention_layernorm_eps(self.find_hparam(["norm_eps"]))
        ...
        if self.find_vparam(["mm_projector_id"]) == "patch_merge":
            self.gguf_writer.add_vision_spatial_merge_size(self.find_vparam(["spatial_merge_size"]))
...
class LazyTorchTensor(gguf.LazyBase):
    ...
    def from_remote_tensor(cls, remote_tensor: gguf.utility.RemoteTensor):
        ...
        lazy = cls(
            meta=meta,
            args=(remote_tensor,),
            func=lambda r: torch.from_numpy(byteswap_tensor(np.frombuffer(r.data(), dtype=numpy_dtype), numpy_dtype))
            .view(dtype)
            .reshape(shape),
        )
        return cast(torch.Tensor, lazy)
