...
class SmoothQuant:
    ...
    def transform_model(
        ...
        if alpha == "auto":  ##TODO need to polish later
            from auto_round.smooth_quant.auto_alpha import TUNERS
        ...
        if example_inputs is not None:
            ...
            if not self.output_is_equal(out_post_sq[0], out_pre_sq[0]):
                logger.warning(
    ...
    def _scale_layer_weight(self, layer_name, scale, alpha=0.5, input_minmax=None):  ##input channel
        ...
        if self.insert_mul:
            from .utils import SQLinearWrapper
            ...
            layer.weight.data = mul_scale(layer.weight, scale)
    ...
    def _absorb_scales(self, layer_name, scale):  ##output channel
        ...
        if (
            ...
            if layer.affine:
                layer.weight.data = mul_scale(layer.weight, scale)
                layer.bias.data = mul_scale(layer.bias, scale)
            else:
                layer.affine = True
            ...
            if hasattr(layer, "bias") and (layer.bias is not None):
                # layer.bias *= scale
                layer.bias = mul_scale(layer.bias, scale)
        ...
        elif layer.__class__.__name__ in ["Qwen2RMSNorm", "Qwen3RMSNorm", "LlamaRMSNorm", "T5LayerNorm"]:
            # layer.weight *= scale
            layer.weight.data = mul_scale(layer.weight, scale)
    ...
    def _adjust_parameters(self, absorb_to_layer, input_maxes, alpha=0.5):
        ...
        for index, key in enumerate(absorb_to_layer.keys()):
            ...
            # if 'norm' not in layer.__class__.__name__.lower():
            #     continue
    ...
    def _parse_absorb_to_layers(self, op_types, folding):
        ...
        if self.insert_mul:
            ...
            group_modules = self._trace(skip_unsupported_layers=False)
    ...
    def _trace(self, skip_unsupported_layers=True):
        """Try the model to find the layers which can be smooth quantized.
