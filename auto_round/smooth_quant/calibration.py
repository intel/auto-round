...
class Calibration:
    def __init__(self, model, dataloder=None, q_func=None, device="cpu", group_size=-1):
        self.model = model
    ...
    def _save_input_pc_hook(self, name):
        ...
        def save_input_hook(module, inputs, outputs):
            if hasattr(module, "orig_layer"):
                weight = module.orig_layer.weight
            else:
                weight = module.weight
            input = inputs[0]
            ##TODO check input channel is correct
            if len(weight.shape) == 4:  ##conv3d or conv1d not supported now, need better way
                input = input.permute(0, 2, 3, 1)
            input, orig_shape, pad_len = reshape_pad_tensor_by_group_size(input, self.group_size)
