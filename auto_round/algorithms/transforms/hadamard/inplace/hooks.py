...
def register_online_had_hooks(model, mapping=None, fp32_had=False, use_fast_had=True):
    ...
    * **o_proj** (``online cross-head had``): cross-head Hadamard on ``num_heads``.
    ...
    """
    if mapping is None:
        from auto_round.algorithms.transforms.hadamard.inplace.model_config import infer_mapping_from_model
...
def get_hadK(n: int, transpose=False) -> (torch.Tensor, int):
    ...
    if is_pow2(n):
        ...
        from auto_round.algorithms.transforms.hadamard.utils.math import _fetch_hadamard_divisor
