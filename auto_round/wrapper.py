...
class WrapperLinear(torch.nn.Module):
    ...
    def _init_tuning_params_and_quant_func(self):
        ...
        self.weight_quant_func, self.data_type = get_quant_func(
            orig_layer.data_type, orig_layer.bits, orig_layer.sym, self.disable_opt_rtn, orig_layer.group_size, self.enable_rtn,
        )
        ...
        if self.enable_act_quant:
            self.act_quant_func, self.act_data_type = get_quant_func(
                orig_layer.act_data_type, orig_layer.act_bits, orig_layer.act_sym, self.disable_opt_rtn, enable_rtn=self.enable_rtn,
            )
