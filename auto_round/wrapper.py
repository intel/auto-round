...
def get_scale_shape(weight, group_size):
    ...
    if isinstance(group_size, list):
        assert len(weight.shape) == len(group_size), f"Expected weight_block_size is {len(weight.shape)}D but get {group_size}"
