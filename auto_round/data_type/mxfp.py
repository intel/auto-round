...
def quant_rtn_mx_rceil(
    ...
    # shared_exp = torch.log2(shared_exp + FP32_MIN_NORMAL * (shared_exp == 0).type(shared_exp.dtype))
    shared_exp = torch.where(max_val == 0, torch.ones_like(max_val), (torch.log2(max_val / max_norm).ceil_()))
    scale_emax = 2.0 ** float(8 - 1) - 1
...
for key in MXFP_FORMAT_CACHE.keys():
    ...
    QUANT_FUNC_WITH_DTYPE["rtn_" + key] = quant_rtn_mx
    QUANT_FUNC_WITH_DTYPE["rtn_" + key + "_rceil"] = quant_rtn_mx_rceil
QUANT_FUNC_WITH_DTYPE["mx_fp_rceil"] = quant_mx_rceil
QUANT_FUNC_WITH_DTYPE["rtn_mx_fp_rceil"] = quant_rtn_mx_rceil
