...
def neuqi_sync_devices(device):
    """Join in-flight GPU work before a search launches.

    The data-driven pipeline's input-caching pass moves activations across
    GPUs on side streams (accelerate). A not-yet-joined producer makes the
    search read stale tensors -- an intermittent device-side assert that
    disappears under CUDA_LAUNCH_BLOCKING=1. One sync per layer-init is noise
    next to the search cost.

    Syncs EVERY visible device, not just this layer's: under accelerate
...
def neuqi_anchor_margins(weight, bits, group_size, imatrix, device, sym=False):
    """Searched (scale, zp) grid for the ``enable_neuqi`` init, expressed as the
    per-group (tensor_min, tensor_max) pair the STE quant functions re-derive
    the grid from.
    ...
    On the tuning path the grid is FROZEN: SignRound tunes the rounding values
    against it, and the range margins stay pinned at 1.0 (the search's grid IS
    the init). On the zero-shot asymmetric path the same margins make the
    plain asym quant reproduce the searched (scale, zp) exactly.

    Returns (wmin, wmax), each squeezed over the trailing group axis.
    """
    import time
    ...
    qw = None
    if imatrix is not None:
        qw = _prepare_neuqi_qw(imatrix, w, bits, group_size).to(device)
    neuqi_sync_devices(device)
    ...
    if sym:
        ...
        wmin = -(zp * s)
        wmax = (maxq - zp) * s
    global _ANCHOR_LOGGED
    if not _ANCHOR_LOGGED:
        _ANCHOR_LOGGED = True
        logger.info(
            "[NeUQI] init-search anchored the quantization grid (%.2fs first layer; margins pinned at 1.0)",
            time.perf_counter() - t0,
        )
    return wmin.squeeze(-1), wmax.squeeze(-1)


def _prepare_neuqi_qw(imatrix, weight_reshape, bits, group_size):
    """Normalize an imatrix to the padded ``[N, gs]`` grouped layout.

    Mirrors the zero-shot search path: the calibration hooks attach a 1D
    ``[in_features]`` per-column importance vector -- it must be padded by
    group size, expanded to the weight's grouped rows, and zero-guarded
    before any per-group search consumes it. ``None`` never reaches here
    (uniform weights).
    """
    from auto_round.data_type.gguf import _imatrix_handle_zero
